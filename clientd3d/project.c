...
// Offset projectile height by this amount.
#define PROJECTILE_SOURCE_HEIGHT_OFFSET - FINENESS / 8
#define PROJECTILE_DEST_HEIGHT_OFFSET - FINENESS / 4
/********************************************************************/
/*
...
void ProjectileAdd(Projectile *p, ID source_obj, ID dest_obj, BYTE speed, WORD flags, WORD reserved)
{
   ...
   p->motion.source_z = s->motion.z + PROJECTILE_SOURCE_HEIGHT_OFFSET; //GetPointFloor(s->motion.x, s->motion.y);
#if 0
   ...
   p->motion.dest_z = d->motion.z + PROJECTILE_DEST_HEIGHT_OFFSET;; //GetPointFloor(d->motion.x, d->motion.y);
#if 0
